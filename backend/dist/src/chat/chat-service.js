...
import { buildDocumentCatalog, buildParsedResultPreview, decideAutoSkillByDocument, ensureAutoSkillOutputPath, } from "./document-router.js";
import { SessionStore } from "./session-store.js";
import { resolveScriptArgPlaceholders } from "../path-manager.js";
import { buildFinancialOutputPaths, decideFinancialIntentByMessage, } from "./financial-intent.js";
/**
 * 多轮会话 + skill 路由编排服务。
 */
export class ChatService {
    ...
    async *chat(sessionId, userMessage) {
        ...
        if (financialIntentDecision) {
            /** 财报意图标签，用于日志与测试断言。 */
            const financialIntent = financialIntentDecision.intent;
            /** 提取与分析输出文件路径。 */
        ...
        else if (autoSkillDecision || (skillDecision.shouldUseSkill && skillDecision.skillName && skillDecision.scriptPath)) {
